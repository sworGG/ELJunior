...
class AuthRepository @Inject constructor(
    ...
    /**
     * Loads full student profile from eluniver.ugrasu.ru via Moodle Web Services.
     */
    suspend fun fetchStudentPersonalData(): Result<StudentPersonalData> {
        val token = getToken() ?: return Result.failure(Exception("Не авторизован"))
        val userId = getUserId() ?: return Result.failure(Exception("ID пользователя не найден"))

        return try {
            val response = moodleApi.getUserProfile(token = token, userId = userId)

            if (!response.isSuccessful) {
                return Result.failure(Exception("Не удалось загрузить личные данные"))
            }

            val user = response.body()?.firstOrNull()
                ?: return Result.failure(Exception("Данные пользователя не найдены"))

            val personalData = StudentPersonalData.fromMoodleUser(user)
            ...
            saveUserData(token, userProfile, getPassword().orEmpty())

            Result.success(personalData)
        } catch (e: Exception) {
            Result.failure(Exception("Ошибка загрузки: ${e.toUserMessage()}"))
        }
    }
}
